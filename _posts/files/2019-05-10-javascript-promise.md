...
호출할수 있는환경이되었을때 호출하는것과 댄을 실행하지 않고 아래 코드로 이동하는 것이 프로미스비동기 처리의핵심입니다.
...
#### 여러 프로미스 실행 (all)
여러개의 비동기 작업들이 존재하고 이들이 모두 완료되었을때 작업을 진행하고 싶다면, 각각의 비동기 작업들을 프로미스에서 처리 하고 여러 프로미스가 모두 완료될 때 all API를 호출합니다. all은 프로미스 객체를 인자로 전달 받습니다.
