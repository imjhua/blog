...
제너레이터 함수로 구성되어있는 saga들은 그 안에 명령을 담고 있는, 이펙트(takeEvery, call, put 등)라 부르는 순수한 객체를 yield 할 것이고, 미들웨어는 이런 명령들을 해석해 처리하고, 그 결과를 다시 Saga에 돌려줍니다. 이러한 작업 중에는 작업에서 직접 수행 할 수있는 것도 있지만, redux-saga에 의뢰하여 간접적으로 수행합니다. 이를 통해 비동기 처리를 co 처럼 기적으로 쓸 수있게하면서 여러 작업을 동시 수행 할 수 있습니다. 

### 이펙트(effect) 함수
- take: 특정 액션을 기다리는 이벤트 발생을 기다린다. 특정 이벤트 액션이 디스패치되기를 기다린다. (pulling)
- takeEvery: 동시에 시작되는 여러개의 액션에 디스패치되기를 기다린다.
- all: 여러개의 이펙트를 처리한다.
- select: store에 저장된 데이터를 미들웨어에서 사용할 수 있도록 한다. State에서 필요한 데이터를 추출할 수 있다.
- put: store에 Action을 디스패치 한다.
- call: Promise의 완료를 기다려, 주어진 함수를 호출한다. 
- fork:	서브 프로세스를 트리거한 뒤 완료를 기다리지 않고 수행한다. (작업중일때 블록하지 않고 제어가 돌아온다. 다른 작업을 시작할 수 있다.)
- cancel:	포크되었던 서브 프로세스를 취소한다.
- cancelled: 현재 프로세스가 켄슬되었었는지 확인한다.
- delay: 다음 구문으로 이동하기 전에 주어진 기간동안 대기한다, Promise를 리턴한다.
- join: 다른 작업이 종료를 기다린다.

#### task()

handleRequestUser작업이 시작되면 즉시 REQUEST_USERAction를 기다리는 take작용을 호출합니다. 이 「기다림」거동이 비동기 처리를 동 기적으로 쓰는 특징적인 작업 계정에 연결됩니다. redux-saga 작업을 Generator 함수 쓰는 이유는 yield의해 처리의 흐름을 일시 중지하는 것입니다. 이 구조 덕분에 단일 스레드 JavaScript로 여러 작업을 시작하여 각각에 특정 Action를 기다리거나, 통신 처리 결과를 기다리고도 처리가 밀릴 수는 없습니다.
...
fork작용은 지정된 작업의 완료를 기다리지 않습니다. 따라서 yield그러자 블록하지 않고 바로 제어가 돌아옵니다. 그러나 rootSaga작업은 handleRequestUser작업의 시작 이외에 할 일이 없습니다. 따라서 rootSaga작업에서 fork를 사용하여 시작한 모든 작업이 완료 될 때까지 기다립니다. 이 동작은 redux-saga v0.10.0에서 도입 된 새로운 실행 모델에 의한 연쇄적인 작업의 취소를 실현하기 위해 필요했습니다. 이것은 부모 작업 자식 작업 손자 작업이 세 가지가 있고, 부모가 아이를 포크하여 아이가 손자를 포크 할 때 부모 작업을 취소하면 제대로 손자 작업까지 취소가 전달 해주는 편리한 기능 입니다. 만약 자식 작업의 완료를 의도적으로 기다리기 싫으면 spawn작용을 사용하여 작업을 시작합니다.
