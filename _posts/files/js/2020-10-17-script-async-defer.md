...
- `<script>`는 HTML 파싱을 블록시킵니다. 스크립트는 fetch 이후에 즉각적으로 실행되며 스크립트가 전부 끝난 이후에 HTML 파싱이 재개됩니다.
- `<script async>`는 HTML 파싱과 함께 병렬적으로 코드가 수행됩니다. (일반적으로는 HTML 파싱이 끝나기 전에 `<script async>`는 사용할 준비가 완료됩니다. analytics와 같은 코드는 `<script async>`와 잘 맞습니다.
- `<script defer>`는 HTML 파싱 중에 병렬적으로 fetch 되고, HTML 파싱이 끝난 이후에 실행됩니다. 만일, `<script defer>` 요소가 여러개 존재한다면, 모든 - `<script defer>`는 순서대로 수행될 것입니다. 만일 스크립트가 완전히 파싱된 DOM 요소에 의존한다면 `<script defer>`를 사용하기 적합합니다. 사실 `<body>`의 끝 부분에 `<script>`를 위치시키는 것과 `<script defer>`를 사용하는 것에 큰 차이는 없습니다.

src 속성이 없는 경우 async와 defer 옵션은 무시당합니다. async와 defer를 지원하는 브라우저는 브라우저가 가지고 있는 JavaScript 엔진마다 약간 다를 수 있습니다.
