...
어떻게 바뀌었나
redux-thunk와 redux-saga 각각의 코드에 대해 자세히 보았다. 여기서 조금 다른 관점에서 생각해보고자합니다. 이 섹션의 시작 부분에서 언급 한 "어디에 쓰는가?" "어디서 호출하는지 '에 대해서입니다.

redux-thunk는 Action Creator 함수를 던지기 때문에 필연적으로 Action Creator에 비동기 처리 코드와 관련된 논리를 움직이지 않게됩니다. 한편 redux-saga는 비동기 처리를 설명하는 전용 구조이다 작업에 씁니다. 그 결과, Action Creator는 본래의 모습을 되찾아, Action 객체를 생성하고 반환 만의 솔직한 녀석으로 돌아갑니다. 개인적으로이 변화는 작지 않다고 생각합니다. 왜냐하면 redux-thunk는 dispatch 된 함수를 붙잡고 오로지 수행하는 특성상 Middleware 스택 (양파 같은 구조이기 때문에 쉘?)의 가장 외측에 배치해야합니다. 그렇지 않으면 다른 Middleware이 함수를 장악되어 오류를 뱉을 수도 있기 때문입니다. 이러한 상황에서 함수가 dispatch 된 것은 redux-thunk 이외 아무도 모르는 사태에 빠지게됩니다. 만약 redux-thunk의 앞에 redux-logger 또는 무언가를 놓았다해도 얻을 수있는 것은 단순한 함수입니다. 내용이 어떻게되어 있는지는 실행하기 전까지는 알 수 없습니다. 최악 이군요. redux-saga 분은 Action Creator가 표준 Action 객체를 생성 할 뿐이므로 redux-logger에서 볼 수 있습니다.

그런데, 조금의 통신 처리는 매우 단순한 것이 었으므로, 낯선 작성을 강제하는 것이 귀찮고별로 혜택을 받았다는 인상이 얇았다지도 모릅니다. 그런 이유로 실제 프로젝트에서 임박한 기능 추가를 해 봅시다. 복잡하게되면 진가를 발휘하는 것이 redux-saga입니다.
