...
SSL기반의 웹 컨텐츠에서 중간에 암호화되지 안ㄴㅎ은 http기반의 컨텐츠가 발생할 경우 브라우저는 http컨텐츠를 로딩하지 않는다. 암호화된 https컨텐츠만 안전하다고 판단하기 떄문에ㅌ
...
1. throttle, debounce 함수를 이용한 해결법

자주 쓰이는 방식으로, 스크롤 이벤트를 이용하게 된다면 throttle이라는 함수를 이용하여 이벤트가 트리거 되는 텀을 조정하고, 리사이즈 이벤트를 이용하게 된다면 debounce라는 함수를 이용하여 이벤트가 완료되고 나면 실행되도록 최적화하게 됩니다. 

(저도 최근까지 이 방법이 최선이라고 ....)

위 두 함수의 동작하는 방식은 아래와 같다.

​

· throttle 이벤트 발생 주기 (설정시간 = 300ms, 굵은 글씨 - 이벤트 발생)

- throttle 미적용 : 이벤트 1 -> 이벤트 2 -> 이벤트 3 -> 이벤트 4 -> 이벤트 5

- throttle 적용(설정된 시간동안 일어난 이벤트는 무시) : 이벤트 1 -> (300ms) -> 이벤트 2 -> (300ms) -> 이벤트 3


​

· debounce 이벤트 발생 주기 (설정시간 = 300ms, 굵은 글씨 - 이벤트 발생)

- debounce 미적용 : 이벤트 1 -> 이벤트 2 -> 이벤트 3 -> 이벤트 4 -> 이벤트 5

- debounce 적용 예 1 : 이벤트 1 -> 이벤트 2 -> (300ms) -> 이벤트 3

- debounce 적용 예 2 : 이벤트 1 -> 이벤트 2 -> (299ms) -> 이벤트 3 -> (300ms) -> 이벤트 4 -> 이벤트 5 -> 이벤트 6 -> (300ms) -> 이벤트 7

​
위와 같이 아까처럼 마구 마구 찍히는 모습이 아닌 것을 볼 수 있습니다. 사실 이정도의 최적화만으로도 최적화 했다고 할 수는 있지만, throttle함수와 debounce 함수의 치명적인 단점은 setTimeout을 기반으로 한 것으로 기대한 결과대로 동작하지 않을 수 있다는 것입니다.

왜냐하면, 싱글 스레드로 동작하는 Javascript 엔진은 setTimout의 비동기 API를 처리하기 위해 task queue에 쌓아서 하나씩 꺼내서 처리하게 되는데 이 과정에서 setTimout 또는 setInterval의 비동기 API가 제대로 trigger 되지 않을 수 있기 때문입니다.
...
렌더링과 레이아웃의 차이는 뭐지.
이게 무슨말이냐면. 그냥 영역으로 그리는거랑 css로 영역을 늘리는 것의 차이...
렌더링? 레이아웃? 트랜스레이트? 캔버스 속성 widht 와 스타일 width가 다른것 처럼
그럼 머는 렌더링이고 머는 레이아웃이냐? 라고 한다면
깨끘한게 레이아웃일거 같은데
그림판 늘리는것은 렌더링 패인팅아닐까?

- clientWidth 실제 크기 요소가 차지하는 실존. 존재. 레이아웃상의 물리적 크기 
- getBoundingClientRect는 변화를 고려하여 렌더링 해서 화면에 그려지는 영역을 담당한다. 
"요소가 화면에서 차지하는 공간의 양" 


또한 clientWidth및 getBoundingClientRect다른 용도로 사용하고 다른 값을 반환 할 수 있습니다. 후자는 또한 변환을 고려하고 실제로 렌더링되는 요소의 크기를 반환합니다.

.class {
  transform: scale(0.5);
}
clientWidth이 경우 1000을 반환 하면 의 너비는 getBoundingClientRect500이됩니다.

clientWidth"요소 내에서 사용할 수있는 getBoundingClientRect공간의 양 "및 "요소가 화면에서 차지하는 공간의 양"으로 간주 할 수 있습니다 . 따라서 우리의 경우 요소는 두 개의 500px 요소를 나란히 포함 할 수있는 충분한 공간을 가지며 화면에서 500px를 차지합니다.

## border-box


참고) 엘리먼트의 속성도 사용할 수 있는데, 테두리를 제외한 엘리먼트의 크기를 알고 싶다면, ClientWidth ClientHeigh를 사용한다.
...
박스의 크기를 어떤 것을 기준으로 계산할지 결정
컨텐츠를 기준으로? 보더를 기준으로?

box-sizing은 박스의 크기를 어떤 것을 기준으로 계산할지를 정하는 속성입니다.

기본값 : content-box -> 패딩과 보더를 제외한 컨텐츠의 크기.
상속 : No
애니메이션 : No
버전 : CSS Level 3
문법

박스모델: content, padding, border, margin이 있다.

box-sizing: content-box | border-box | initial | inherit
content-box : 테두리와 패딩을 제외한, 콘텐트 영역(패딩까지만)을 기준으로 크기를 정합니다.
...
// box-sizing를 border-box로 지정하면, -> 패딩과 보더를 포함한 크기
...
/* 
#show-page-0 .page-0 .sticky,
#show-page-1 .page-1 .sticky,
#show-page-2 .page-2 .sticky,
#show-page-3 .page-3 .sticky {
  display: block;
...
JavaScript를 사용하여 다른 Origin으로 _<iframe>_에 액세스하면 cantt 액세스 할 수 있습니다. 동일 출처 정책 브라우저는 다른 Origin 로 프레임에 액세스하려는 스크립트를 차단합니다.
