...
## 리엑트
재사용성이 없는 컴포넌트를 각각의 파일로 작성해두어야 하는 별도의 이유가 있을까요..?
 결론부터 말씀드리면 가독성을 위해서 입니다.
 하나의 컴포넌트 파일에서 구조는 다음과 같습니다.
...
여기서 여러개의 컴포넌트가 하나의 파일에 존재한다면 이러한 구조가 반복되게 되고 어느것이 부모에 해당하는 컴포넌트이며 어느것이 외부에 재사용이 되지 않는 컴포넌트인지 구분도 모호해지게 됩니다. 따라서 하나의 파일에 하나의 컴포넌트를 작성하는 것을 권고 드린 것입니다.
 현재도 페이지별로 구분되어 컴포넌트가 구성되고 컴포넌트 내부에도 각각의 컴포넌트들로 구현되어 있는 것들이 있습니다. 그것들을 각각의 파일이나 directory로 구분하지 않고 하나의 파일로 작성했을 때 복잡해 질 것이다라고 생각 하실 것 같은데 그것과 동일한 이유라고 생각하시면 이해하기 편하실까요?




 HOC에 대하여
HOC는 하나의 부품처럼 특정 컴포넌트에 기능 하나를 추가해주는 역할을 한다고 생각하는데요. 그것의 좋은 활용 예시가 withLoading HOC입니다. 로딩을 띄우는 변수명에 따라서 컴포넌트에 로딩을 ON/OFF 시켜주는 기능을 추가하는 것 입니다.
이 경우에 대해서는 특정 케이스별로 다르게 리턴을 하는 것을 목적으로 HOC를 사용한 것이라면, HOC를 사용하지 않고 Recive 컴포넌트 내부에 case를 넣어 녹여내서 분기별로 다르게 리턴하게 구현하면 된다고 생각합니다.
생각이 다르시거나 이해가 안되시는 부분이 있으면 답변주세요!


정리하자면 store의 구조가 async action에 1:1로 대응되는 것이 아니라 데이터의 성격에 따라서 store에 저장을 해놓고 공통으로 사용하는것이 어떻느냐는 질문이라고 이해했습니다. 맞나요?
맞다는 가정하에 제 의견을 말씀드리겠습니다.
우선 좋은 의견이고 동의합니다.
중복되는 데이터들이라면 한곳에서 모아서 관리를 하는 것이 맞다고 생각합니다.
다만, action에 맞추어 status를 관리하고 response를 저장한다는 것은 component 에서 action의 상태에 따른 후처리를 대응하려고 하는 생각이 있습니다.
이번에 전달한 코드스타일을 적용함으로써 얻고 싶은 장점이 있다면
1.  component에서 어떠한 액션과 관련있는지 알고 싶다.
2. 액션의 상태 및 결과(success, response)에 대해서 후처리를 component에서 하고 싶다.
이 두가지 정도가 있습니다.
이 두가지를 해치지 않는다면 괜찮다고 생각하는데 답변이 되었을까요??
