...
### NaN(Not-A-Number)
JavaScript에서 NaN 값은 not a number라는 뜻으로, 즉 숫자가 아니라는 의미로 해석됩니다. 그러나 NaN은 매우 특이하게 작동 되는데 typeof로는 NaN과 숫자(number)를 구분할 수가 없고, 자기 자신과의 비교도 불가 합니다.
...
그 이유는 원시값이 가지고 있는 constructor 의 값이 String이 아니기 때문입니다. 다음을 확인해 보면 그 차이를 알 수 있습니다.
```js
